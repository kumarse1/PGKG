...
def enrich_answer_with_context(answer, nodes_df):
    ...
    for _, row in terms.iterrows():
        ...
        if name and name.lower() in answer.lower():
            label = row["label"]
            bullets.append(f"• **{name}** is a **{label}** related to the knowledge graph.")
...
selected_label = st.selectbox("Step 1: Choose a Category", labels)
filtered_nodes = nodes_df[nodes_df.label == selected_label]
node_options = ["All"] + sorted(filtered_nodes["name"].unique())
selected_node = st.selectbox("Step 2: Choose a Node (or view all)", node_options)
...
if is_cat_view:
    ...
    direction_label = st.selectbox("Step 3: Choose connection direction", [
        "All connections", "Only shows what this connects to", "Only shows what connects to this"
    ])
    direction_map = {
