...
# NEW ADDITION: Helper function to get FULL graph summary for comprehensive context
def get_full_graph_summary(nodes_df, edges_df, max_examples=3):
    """
    NEW FUNCTION: Gets complete graph overview for LLM context
    Always provides full graph access regardless of current selection
    """
    try:
        summary = []
        
        # Get all categories and their counts
        categories = nodes_df.groupby('label').size().to_dict()
        summary.append("COMPLETE KNOWLEDGE GRAPH:")
        for category, count in categories.items():
            summary.append(f"- {category}: {count} nodes")
        
        # Sample relationships from each category (limit for token efficiency)
        summary.append("\nKEY RELATIONSHIPS:")
        for category in categories.keys():
            category_nodes = nodes_df[nodes_df.label == category].head(max_examples)
            for _, node in category_nodes.iterrows():
                # Get a few connections for this node
                out_edges = edges_df[edges_df.source_node_id == node.node_id].head(2)
                for _, edge in out_edges.iterrows():
                    ...
                    if not target.empty:
                        ...
                        summary.append(f"  {node['name']} ({category}) → {target_name} ({target_label}) via '{edge.relation_type}'")
        
        return "\n".join(summary)
        
    except Exception as e:
        return f"Error getting full graph summary: {str(e)}"
...
def query_llm(question, context_data):
    ...
    try:
        ...
        # ALWAYS provide full graph access
        full_graph_data = get_full_graph_summary(
            context_data.get("nodes_df"), 
            context_data.get("edges_df")
        )
        
        # Add current selection context if relevant
        if "selected_node" in context_data and context_data["selected_node"]:
            if context_data.get("direction") == "Category Overview":
                current_focus = f"\nCURRENT VIEW: Category overview of {context_data['selected_node']}"
            else:
                current_focus = f"\nCURRENT VIEW: Focused on node '{context_data['selected_node']}'"
        else:
            current_focus = "\nCURRENT VIEW: General graph view"

        # Format context - CONCISE hybrid approach
        context_text = f"""
        {full_graph_data}
        {current_focus}

        INSTRUCTIONS: 
        - For graph questions: Use the complete knowledge graph data above
        - For general questions: Give brief, helpful explanations (2-3 sentences max)
        - Always check the full graph first, then add context if needed
...
if len(filtered_nodes) <= 3:
    # If 3 or fewer nodes, show all + individual nodes
    node_options = ["All"] + sorted(filtered_nodes["name"].unique())
    selected_name = st.selectbox("Select Specific Node", node_options, help="Choose All to see everything or specific node")
    show_category_view = (selected_name == "All")
else:
    # If many nodes, offer category overview option
    node_options = ["All", "📊 Category Overview"] + sorted(filtered_nodes["name"].unique())
    selected_name = st.selectbox("Select Node", node_options, help="Choose All, category overview, or specific node")
    show_category_view = (selected_name in ["All", "📊 Category Overview"])
...
if st.button("Send") and question.strip():
    ...
    with st.spinner("Thinking..."):
        # Simple context - let smart function handle the rest
        context_data = {"nodes_df": nodes_df, "edges_df": edges_df, "selected_node": selected_name}
